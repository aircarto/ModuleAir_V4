...
- Web UI : auto-refresh du dashboard toutes les 60s pour pull les nouvelles mesures et refleter les changements d'etat (badges, toggles grises). Pause automatique si l'utilisateur est en train de saisir dans un champ ou a interagi dans les 5 dernieres secondes pour ne pas couper une edition en cours.
- Web UI : toggle capteur declenche un reload immediat de la page (au lieu d'attendre les 60s de l'auto-refresh) pour que les effets en cascade (badges, locks d'ecrans, cartes de donnees) soient visibles instantanement apres clic.
